...
class Config extends Object {
	...
	static public function read($file="environment.ini"){
		...
		if($file=="environment.ini"){
			$core = Config::read("config.ini");
			$active_app = Kumbia::$active_app;

			if(!isset($core->$active_app->mode)){
				if(!isset($core->$active_app)){
					$core->$active_app = new stdClass();
				}
				$core->$active_app->mode = "production";
			}

			//Carga las variables db del modo indicado
			if(isset($config->{$core->$active_app->mode})){
				foreach($config->{$core->$active_app->mode} as $conf => $value){
				...
				throw new KumbiaException("El entorno '{$core->$active_app->mode}' no existe en config/enviroment.ini");
			}
