...
function age(){
	...
	if(!isset($params['order'])){
		if($kumbia_config = Config::read('core.ini')){
			if(preg_match('/^DD[^DMY]MM[^DMY]YYYY$/', $kumbia_config->kumbia->dbdate)){
				$params['order'] = 'd-m-Y';
			} elseif(preg_match('/^DD[^DMY]YYYY[^DMY]MM$/', $kumbia_config->kumbia->dbdate)){
				$params['order'] = 'd-Y-m';
			} elseif(preg_match('/^MM[^DMY]DD[^DMY]YYYY$/', $kumbia_config->kumbia->dbdate)) {
				$params['order'] = 'm-d-Y';
			} elseif(preg_match('/^MM[^DMY]YYYY[^DMY]DD$/', $kumbia_config->kumbia->dbdate)) {
				$params['order'] = 'm-Y-d';
			} elseif(preg_match('/^YYYY[^DMY]DD[^DMY]MM$/', $kumbia_config->kumbia->dbdate)) {
				$params['order'] = 'Y-d-m';
			} else {
				$params['order'] = 'Y-m-d';
			}
...
/**
	...
	*Para paginacion por sql:
	...
	Retorna un PageObject que tiene los siguientes atributos:
		next_page: numero de pagina siguiente, si no hay pagina siguiente entonces es false
		before_page: numero de pagina anterior, si no hay pagina anterior entonces es false
		current_page: numero de pagina actual
		total_pages: total de paginas que se pueden mostrar
...
function paginate() {
	...
	if(is_array($params[0])) {
		...
		**/
		if(isset($params['conditions'])) {
			...
			array_push($find_args, "conditions: $params[1]");
		}
		...
		**/
		...
		/**
			Hallamos el total de filas en la tabla
		**/
		if(isset($find_args['conditions'])) {
			$n = $model->count($find_args['conditions']);
		} else {
			$n = $model->count();
		}
	...
	**/
	$page->next_page = ($start + $per_page)<$n ? ($page_number+1) : false ;
	$page->before_page = ($page_number>1) ? ($page_number-1) : false ;
	$page->current_page = $page_number;
	$page->total_pages = ($n % $per_page) ? ((int)($n/$per_page) + 1):($n/$per_page);
