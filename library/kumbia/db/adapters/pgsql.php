...
class DbPgSQL extends DbBase implements DbBaseInterface {
	...
	/**
	 * Nombre del usuario en PostgreSQL
	 *
	 * @var string
	 */
	private $db_user;
	...
	/**
	 * Password del Usuario en PostgreSQL
	 *
	 * @var string
	 */
	private $db_pass;
	...
	/**
	 * DSN de conexi&oacute;n a PostgreSQL
	 *
	 * @var string
	 */
	private $db_dsn;
	...
	public function connect($dbhost='', $dbuser='', $dbpass='', $dbname='', $dbport='', $dbdsn=''){
		...
		if(!$dbhost) $dbhost = $this->db_host; else $this->db_host = $dbhost;
		if(!$dbuser) $dbuser = $this->db_user; else $this->db_user = $dbuser;
		if(!$dbpass) $dbpass = $this->db_pass; else $this->db_pass = $dbpass;
		if(!$dbport) $dbport = $this->db_port; else $this->db_port = $dbport;
		if(!$dbname) $dbname = $this->db_name; else $this->db_name = $dbname;
		if(!$dbdsn) $dbdsn = $this->db_dsn; else $this->db_dsn = $dbdsn;

		if($this->id_connection = pg_connect("host={$this->db_host} user={$this->db_user} password={$this->db_pass} dbname={$this->db_name} port={$this->db_port}")){
			return true;
		} else {
